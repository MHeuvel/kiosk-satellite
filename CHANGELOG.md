...
- **Approve Spanish camera, app-list and plugin error translations.** Import the reviewed catalog with all 3148 Spanish messages approved and no missing or stale entries.

- **Translate camera, app-list and plugin failures.** Overview reuses Camera and Camera Streams errors. App selection and plugin repository, package, compatibility and update failures use shared error formatters on the device and in remote administration. Open remote error dialogs follow language changes without repeating actions or enabling incompatible installs.
...
- **Kiosk Mode and Home Launcher use translation templates in both interfaces.** Six templates add 83 messages for controls, allowed actions, permissions, PIN prompts and home-role status. All 1989 catalog messages now have bundled, author-approved Spanish translations, using "Lanzador de Inicio" for Home Launcher. Language changes preserve PIN input and saved values.
- **Intercom settings and call controls use translation templates.** Seven templates add 141 messages for setup, keys, kiosk status, calls, announcements and shared sound pickers. All 1906 catalog messages now have bundled, author-approved Spanish translations. Longer controls fit narrow screens and the key dialog keeps its field alive through its closing animation.
...
- **Camera settings now use translation templates in both interfaces.** Six templates add 127 messages for camera controls, motion sensing, RTSP and ONVIF streaming, permissions, connected clients and remote snapshots. All 1397 catalog messages now have bundled, author-approved Spanish translations. Translated controls preserve camera selections, addresses and stream settings.
- **Screensaver detection settings now use translation templates in both interfaces.** Motion, Face, Proximity and Person Detection share 75 new messages across six templates, including camera preview controls, hardware availability, permissions and live occupancy status. All 1270 catalog messages now have bundled, author-approved Spanish translations. Translated notices preserve sensor names, stored settings and ADB commands.
- **Screensaver Widgets and At a Glance now use translation templates in both interfaces.** Six templates add 106 messages for appearance, weather, entity search and editing with author-approved Spanish translations. All 1195 catalog messages now have bundled Spanish wording. Translated editors preserve corner values, font choices, entity IDs and custom names. Saving a custom name also waits for the dialog to close before disposing its text controller.
- **Screensaver media settings now use translation templates in both interfaces.** Home Assistant Media, Local Media, Photo Gallery, Immich and Camera Streams share 205 new catalog messages with author-approved Spanish translations. All 1089 catalog messages now have bundled Spanish wording, using "Immich" for its mode label and page name. Media pickers, connection results, filters, calendars and ordering controls preserve external names and saved values.
- **Screensaver controls, Clock settings and schedules now use the translation catalog.** All 884 catalog messages now have bundled, author-approved Spanish translations. Both interfaces share 174 new messages for the main controls, Dim, Black and Website modes, Clock appearance, schedule overrides and shared time and color pickers. Translated editors preserve plugin names and stored values.
- **Screen & Audio now has translation templates in both interfaces.** Screen, adaptive brightness, volume, audio device and microphone controls share 93 new catalog messages with author-approved Spanish translations. All 710 catalog messages now have bundled Spanish wording. Translated choices preserve hardware selectors and saved values.
- **Home Assistant Setup now has translation templates in both interfaces.** Connection validation, proxy guidance, dashboard selection and the User Interface, Theme, rotation, return-to-home, Hold mode and Optimizations subpages use the catalog. All 617 catalog messages now have bundled, author-approved Spanish translations using "panel de control" for dashboard. Dashboard names, entity names and stored paths remain unchanged.
- **Device settings now have translation templates in both interfaces.** The catalog covers User Interface, remote administration, configuration import/export, permissions, service status, updates, Shizuku, the optional update helper and analytics. Spanish translations are imported from a reviewed snapshot tied to the committed English source. Translated option labels keep their original saved values.
- **Settings navigation and the device drawer now use localization catalogs.** Menu names, summaries, search controls and conditional drawer actions have translation templates. Spanish wording covers hold mode, player controls, confirmations and update notices, using "protector de pantalla" for screensaver. Settings search accepts translated menu names and English aliases.
- **Choose the kiosk's language under Device > User Interface.** Language is the first row in the group and offers English and Español, with English selected by default. Changes apply immediately on the kiosk and in remote administration and remain local to each kiosk. Approved Spanish catalog messages are bundled, including Settings navigation, search and drawer actions. Screens outside the catalog remain in English.
- **Localization starts with setup and shared connection settings.** English messages now have translation templates shared by the device and remote admin. Templates follow visible setup pages and settings sections with common actions in their own file, exact UI paths and separate translator notes. Catalog validation checks IDs across files, section membership and placeholders. Reviewed Spanish translations can be imported from a pinned localization commit with English fallback for missing or stale messages. Spanish is enabled only after importing a reviewed snapshot.
