...
### Added
- **An intercom between kiosks.** Kiosks on the same network can call each other and talk to every kiosk at once, with the voice going straight between them over the remote admin port and no server in the path. A new Intercom page on the device and in the remote admin holds the switch, a shared Intercom key (made on first enable, pasted between kiosks or synced by Fleet Management as a credential), the Answer mode (Ring, Answer automatically after one ring and a countdown, or Do not disturb), Ring for, the Ring sound (a built-in telephone ring or a file from the sounds folder), a Show in the kiosk menu switch, the Talk mode (Push to talk by default, Hands free with Mute on devices with an echo canceller) and the Intercom volume, plus a Kiosks card that says which kiosk is Ready, off, on another key, on Do not disturb or offline. The kiosk menu gains an Intercom entry with its own Allowed Actions switch, Gestures an Open the intercom action, the remote Overview a Do not disturb tile and Home Assistant four ESPHome entities: the state, the other kiosk's name, a Do not disturb switch and the answer mode. Announce to all in the kiosk menu talks to every kiosk one way, and Home Assistant announces through the intercom with the intercom_announce ESPHome action: a message its text to speech speaks or an audio URL, to one kiosk by IP address or to all, with an override for Do not disturb that an Accept announcements switch on the receiver still beats. A call wakes the screen, draws over the screensaver, holds the ambient features and ducks the music like a voice turn and pauses wake word detection. Lockdown Mode answers as Do not disturb, a second caller gets Busy and a missed call is a toast with Call back.
