...
def generate(app, preview_repo=None):
    ...
    (output / "language_codes.dart").write_text(
        ...
        + "];\n")
    nav_path = app / "l10n/navigation.json"
    navigation = read(nav_path) if nav_path.exists() else {}
    for label, identifier in navigation.items():
        if identifier not in messages(source) or source[identifier] != label or parameters(label):
            raise ValueError(f"Invalid navigation message mapping: {label}")
    (output / "navigation_ids.dart").write_text(
        "// Generated by tool/localization.py. Do not edit.\n"
        + "const navigationMessageIds = <String, String>"
        + json.dumps(navigation, ensure_ascii=False, indent=2) + ";\n")
    (app / "remote-ui/static/navigation_ids.js").write_text(
        "// Generated by tool/localization.py. Do not edit.\nexport const navigationMessageIds = "
        + json.dumps(navigation, ensure_ascii=False, indent=2) + ";\n")
