...
The unmodified ES module builds are vendored in `app/remote-ui/static/vendor-marked.js` and `vendor-purify.js`. They were obtained from the corresponding versioned npm package tarballs. Keep the version, source and license notices together when updating them. The build minifies both for the APK and puts each file's copyright banner back on top of the minified copy.
