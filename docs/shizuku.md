...
Allowed action IDs are `identity`, `reboot`, `grantAll`, `microphone`, `batteryUnrestricted`, `camera`, `bluetooth`, `notification`, `displayOverOtherApps`, `writeSettings`, `uiGuard`, `deviceAdmin`, `allFiles`, `usageAccess` and `location`. The batch includes all 12 permissions regardless of feature settings and skips existing grants. Callers cannot supply package names or shell commands.
