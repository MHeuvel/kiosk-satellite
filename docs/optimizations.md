...
The Home Assistant status pill on the remote admin Overview also shows **Watching N entities** when a filter is active or **Updates unfiltered** with an amber dot and status text when the view runs unfiltered. It uses the same warning style as the App Version tile. With **Filter dashboard updates** disabled, it shows only **Validated** with the normal green style. The count uses the existing allowlist and is refreshed at most once every 30 seconds while Overview is visible.
